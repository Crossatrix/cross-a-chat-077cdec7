...
/**
 * Icon names mods may reference in tabs.json's `icon` field. Kept as an explicit
 * allowlist (rather than trusting arbitrary strings) so a mod can only select from
 * icons this app actually ships with lucide-react — see MOD_TAB_ICON_MAP in Index.tsx
 * for the matching component map.
 */
export const MOD_TAB_ICON_NAMES = [
...
export type ModTabIconName = typeof MOD_TAB_ICON_NAMES[number];
