...
export interface ModTab { id: string; name: string; ui: string; modId: string; }
...
const parseTabs = (raw: string, uiPaths: Set<string>): { id: string; name: string; ui: string }[] => {
  const out: { id: string; name: string; ui: string }[] = [];
  let parsed: unknown;
  try { parsed = JSON.parse(raw); } catch { return out; }
  if (!Array.isArray(parsed)) return out;
  for (const entry of parsed) {
    if (!entry || typeof entry !== "object") continue;
    const { id, name, ui } = entry as Record<string, unknown>;
    ...
    out.push({ id: id.trim(), name: name.trim(), ui: normalized });
  }
  return out;
};
