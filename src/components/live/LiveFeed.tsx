            ...
            {streams.map((s) => (
              <button key={s.id} onClick={() => setSelected(s)}
                ...
                <div className="p-2.5 flex gap-2">
                  <FeaturedAvatar userId={s.user_id} avatarUrl={s.profiles?.avatar_url} username={((s.profiles as any)?.creator_username || s.profiles?.username || "deleted")}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-xs text-muted-foreground truncate">{((s.profiles as any)?.creator_username || s.profiles?.username || "deleted")}</span>
                    </div>
