...
const VideoLeaderboard = ({ onSelectVideo, onCreatorClick }: VideoLeaderboardProps) => {
  ...
  const getRankIcon = (index: number) => {
    if (index === 0) return <ModImg src={crownImg} alt="Crown" className="h-5 w-5 object-contain" />;
    if (index === 1) return <ModImg src={top2Img} alt="2nd Place" className="h-5 w-5 object-contain" />;
    if (index === 2) return <ModImg src={top3Img} alt="3rd Place" className="h-5 w-5 object-contain" />;
