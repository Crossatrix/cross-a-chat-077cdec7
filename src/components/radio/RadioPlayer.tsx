...
export default function RadioPlayer({ channelId, channelName }: Props) {
  ...
  useEffect(() => {
    ...
    load();
    const ch = supabase
      .channel(`radio-now-playing-${channelId}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "radio_now_playing", filter: `channel_id=eq.${channelId}` },
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold">{channelName || "Cross Radio"} {isLive && <span className="ml-2 inline-flex items-center gap-1 text-xs text-red-500"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />LIVE</span>}</span>
      </div>
