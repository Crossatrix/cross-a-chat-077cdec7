...
import { getModTabs, onModsUpdated, type ModTab } from "@/utils/mods";
...
/**
 * Resolves a tabs.json `icon` name to its component from the full lucide-react
 * namespace. Any icon that exists in the installed lucide-react version works;
 * mods.ts's MOD_TAB_ICON_NAMES (derived the same way) is what actually validates
 * the name at install time, so by the time we get here `tab.icon` is already safe
...
return (
    <>
      ...
      <div className="order-last md:order-none flex border-t md:border-t-0 md:border-b border-border bg-card shrink-0 z-10">
        ...
        {modTabs.map((tab) => {
          const TabIcon = resolveModTabIcon(tab.icon);
          return (
            <button
              key={tab.id}
