...
mod.log("messages:", msgs);`}</pre>
            ...
            <div>
              ...
              <ul className="list-disc pl-5 text-xs space-y-1">
                ...
                <li><code>mod.toast(msg)</code>, <code>mod.log(...)</code></li>
                ...
                <li><code>mod.confirm(msg)</code>, <code>mod.prompt(msg, default?)</code> — native dialogs</li>
                <li><code>await mod.copy(text)</code> — copy text to clipboard</li>
                <li><code>mod.navigate(path)</code> — redirect the page</li>
                <li><code>await mod.sleep(ms)</code> — async delay helper</li>
                <li><code>mod.notify(title, body?)</code> — richer toast with a description</li>
                <li><code>await mod.getBalance()</code> — current user's Croins balance</li>
                <li><code>await mod.listPosts(limit?)</code> — recent posts feed</li>
                <li><code>await mod.likePost(postId)</code> — like a post as the current user</li>
                <li><code>mod.listInstalledMods()</code> — id/name/description of other installed mods</li>
                <li><code>mod.listTabs()</code> — id/name of custom tabs added via <code>tabs.json</code></li>
...
[event: openedsettings; run{UI/panel.html}]`}</pre>
              ...
              <p className="text-xs font-mono break-words">
                ...
                changegrouprole, joingroup, changesetting, watchvideo, logout, leavegroup, unblockuser,
                comment, editpost, deletepost, openedstore, openedadmin, uploadvideo, sendcroins
              </p>
            </div>

            <div>
              <h2 className="font-semibold text-base mb-1">tabs.json</h2>
              <p>
                Adds one or more custom tabs to the main tab bar. Each tab renders its <code>ui</code> file
                inline (not in a dialog) using the same sandboxed <code>&lt;iframe&gt;</code> and
                <code> window.mod</code> bridge as regular UI screens. The file is a JSON array:
              </p>
              <pre className="bg-muted rounded p-3 text-xs overflow-x-auto">{`[
  { "id": "b6f1a1b0-2e3b-4a1a-9c3f-4e2b6a2f9d10", "name": "My Tab", "ui": "UI/mytab.html", "icon": "Star" },
  { "id": "0b6c9a1e-7f3d-4c2a-8e9b-1a2b3c4d5e6f", "name": "Stats", "ui": "UI/stats.html", "icon": "BarChart3" }
]`}</pre>
              <ul className="list-disc pl-5 text-xs space-y-1">
                <li><code>id</code> — any unique UUID (or string) you generate yourself; identifies the tab</li>
                <li><code>name</code> — label shown under the icon in the tab bar</li>
                <li><code>ui</code> — path to an HTML file already included under <code>UI/</code></li>
                <li><code>icon</code> — optional; the exact PascalCase name of any icon from{" "}
                  <a href="https://lucide.dev/icons" target="_blank" rel="noreferrer" className="underline">
                    lucide.dev/icons
                  </a>{" "}
                  (e.g. <code>Star</code>, <code>BarChart3</code>, <code>Gamepad2</code>). Every icon in
                  the installed lucide-react version is available — falls back to a generic package icon
                  if omitted or not recognized</li>
              </ul>
              <p className="mt-2">
